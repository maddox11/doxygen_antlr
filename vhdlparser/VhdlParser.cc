...
void VhdlParser::architecture_body() {QCString s,s1;Token *tok=0;
    if (!hasError) {
...
outlineParser()->error_skipto(BEGIN_T);
    ...
    if (!hasError) {
    tok = jj_consume_token(SEMI_T);
    }
    if (!hasError) {
outlineParser()->setEndBodyLine( m_sharedState->lastCompound->startLine,tok->beginLine);
    }
...
void VhdlParser::component_declaration() {QCString s;Token *tok=0;int l;
    if (!hasError) {
    ...
    if (!hasError) {
m_sharedState->currP=VhdlSpecifier::COMPONENT; l=outlineParser()->getLine(COMPONENT_T);
    }
    if (!hasError) {
    switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
    case GENERIC_T:{
...
outlineParser()->setEndBodyLine( l,tok->beginLine);
}
...
void VhdlParser::entity_declaration() {QCString s;Token* tok;
    if (!hasError) {
    jj_consume_token(ENTITY_T);
    }
    if (!hasError) {
...
m_sharedState->lastEntity=m_sharedState->current;
    ...
    if (!hasError) {
outlineParser()->setEndBodyLine( m_sharedState->lastEntity->startLine,tok->beginLine);
    }
m_sharedState->lastEntity=0;m_sharedState->lastCompound=0; m_sharedState->genLabels.resize(0);
}
...
void VhdlParser::package_body() {QCString s;Token *tok=0;
    if (!hasError) {
    jj_consume_token(PACKAGE_T);
    }
    if (!hasError) {
...
void VhdlParser::package_declaration() {QCString s;Token *tok=0;
    if (!hasError) {
    jj_consume_token(PACKAGE_T);
    }
    if (!hasError) {
    ...
    if (!hasError) {
m_sharedState->lastCompound=m_sharedState->current;
                          m_sharedState->lastCompound->section=EntryType::makeNamespace();
                          m_sharedState->lastCompound->startLine=outlineParser()->getLine(PACKAGE_T);
