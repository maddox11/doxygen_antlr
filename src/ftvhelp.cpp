...
static std::string filterIcon(const std::string s)
{
}

static std::string compoundIcon(const std::string s)
{
  ...
  if (s.find("program")!=np)
  {
    return "Pr";
  }

  if (s.find("checker")!=np)
  {
    return "Ch";
  }

  if (s.find("interface")!=np)
  {
    return "I";
  }

  size_t u = s.find("input");
...
void FTVHelp::Private::generateTree(TextStream &t, const FTVNodes &nl, int level, int maxLevel, int &index)
{
  for (const auto &n : nl)
  {
    std::string lk = n->name.data();
    std::string a1, b1, c1, d1, e1;
    const Definition *kl = n->def;
    a1 = kl->displayName().data();
    b1 = kl->symbolName().data();
    c1 = kl->qualifiedName().data();
    d1 = kl->getReference().data();
    e1 = kl->getOutputFileBase().data();
    std::string pre = n->ref.data();
    std::string pp = n->anchor.data();
    // printf("[[  %s %s  ]]\n", lk.data(), e1.data());
    ...
    if (n->isDir)
    {
      ...
      else if (n->def && n->def->definitionType() == Definition::TypeClass)
      {
        ...
        std::string icon = compoundIcon(e1);
                           ...
                           const FTVNodes &nl, int level, bool &first)
{
  ...
  for (const auto &n : nl)
  {
    ...
    else // show items in this file
    {
      ...
      const MemberDef *mdd;
      std::string pic;
      std::string tic = "plus.svg";
      // if (n->def && n->def->definitionType() == Definition::TypeMember)

      if (n->def && (n->def->definitionType() == Definition::TypeMember || n->def->definitionType() == Definition::TypeClass))
      {
        StringVector sv;
        bool dClass = (n->def->definitionType() == Definition::TypeClass);
       // std::cout << n->def->name() << ".....\n";

        if (dClass)
        {
          const ClassDef *iii = toClassDef(n->def);
          sv = iii->getQualifiers();
          isMem = 1;
        }
        else
        {
          isMem = 2;
          const MemberDef *iii = toMemberDef(n->def);
          sv = iii->getQualifiers();
          if (!sv.empty())
          {
            std::string val = sv.at(0);
            tic = compoundIcon(val);
          //  printf("memberquali:  %s  %s %s\n", iii->name().data(), sv.at(0).data(), tic.data());
          }
        }
        if (!sv.empty())
        {
          pic = sv.front();
          // printf("{%s %s }\n", iii->name().data(), pic.data());
        }
      }
...
static void generateJSTreeFiles(NavIndexEntryList &navIndex, TextStream &t, const FTVNodes &nodeList)
{
  ...
  if (numThreads > 1) // multi threaded version
  {
    ...
    {
      results.emplace_back(threadPool.queue([&]()
                                            { generateJSFile(tf); }));
    }
  ...
  if (f.is_open())
  {
    ...
    if (tsidx.is_open())
    {
      ...
      while (it != navIndex.end())
      {
        ...
        tsidx << "\"" << e.url << "\":[" << e.path << "]";
        ++it;
        if (it != navIndex.end() && elemCount < maxElemCount - 1)
