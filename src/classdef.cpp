...
static QCString getCompoundTypeString(SrcLangExt lang, ClassDef::CompoundType compType, bool isJavaEnum)
{
  ...
  else
  {
    ...
    case ClassDef::Union:
      return "union";
    case ClassDef::Module:
      return "module";
    case ClassDef::Interface:
      return lang == SrcLangExt_ObjC ? "class" : "interface";
    case ClassDef::Protocol:
      return "protocol";
    case ClassDef::Category:
      return "category";
    case ClassDef::Checker:
      return "checker";
    case ClassDef::Package:
      return "package";
    case ClassDef::Program:
      return "program";
    case ClassDef::Configuration:
                                  ...
                                  Specifier s, const QCString &t)
{
  ...
  if (!extractPrivate && cd->protection() == Protection::Private)
    return;
...
static void writeInheritanceSpecifier(OutputList &ol, const BaseClassDef &bcd)
{
  if (bcd.prot != Protection::Public || bcd.virt != Specifier::Normal)
  {
    ...
    if (bcd.prot == Protection::Protected)
      sl.push_back("protected");
    else if (bcd.prot == Protection::Private)
      sl.push_back("private");
    if (bcd.virt == Specifier::Virtual)
...
void ClassDefImpl::writeInheritanceGraph(OutputList &ol) const
{
  ...
  if (classGraph == CLASS_GRAPH_t::NO)
    return;
...
void ClassDefImpl::writeIncludeFiles(OutputList &ol) const
{
  ...
  {
    SrcLangExt lang = getLanguage();
    QCString nm = m_impl->incInfo->includeName.isEmpty() ? (m_impl->incInfo->fileDef ? m_impl->incInfo->fileDef->docName() : QCString()) : m_impl->incInfo->includeName;
    if (!nm.isEmpty())
    {
      ol.startParagraph();
      ol.startTypewriter();
...
{
  if (!isLinkableInProject() || isArtificial())
    return;
  ...
  for (const auto &lde : LayoutDocManager::instance().docEntries(LayoutDocManager::Class))
  {
    ...
    case LayoutDocEntry::ClassNestedClasses:
    {
      for (const auto &innerCd : m_impl->innerClasses)
      {
        ...
        {
          tagFile << "    <class kind=\"" << innerCd->compoundTypeString() << "\">" << convertToXML(innerCd->name()) << "</class>\n";
        }
      }
    }
    break;
  ...
  for (const auto &lde : LayoutDocManager::instance().docEntries(LayoutDocManager::Class))
  {
    ...
    case LayoutDocEntry::MemberDecl:
    {
      ...
      if (lmd)
      {
        ...
        if (!isSimple)
          writeMemberDeclarations(ol, visitedClasses, lmd->type, lmd->title(lang), lmd->subtitle(lang), TRUE);
      }
    }
    break;
    case LayoutDocEntry::MemberGroups:
...
void ClassDefImpl::addClassAttributes(OutputList &ol) const
{
  ...
  if (isSealed())
    sl.push_back("sealed");
  if (isAbstract())
    sl.push_back("abstract");
  if (isExported())
    sl.push_back("export");
  if (getLanguage() == SrcLangExt_IDL && isPublished())
    sl.push_back("published");
...
void ClassDefImpl::writeMemberList(OutputList &ol) const
{
  ...
  bool sliceOpt = Config_getBool(OPTIMIZE_OUTPUT_SLICE);
  bool generateTreeView = Config_getBool(GENERATE_TREEVIEW);
  if (m_impl->allMemberNameInfoLinkedMap.empty() || cOpt)
    return;
  // only for HTML
  ol.pushGeneratorState();
  ol.disableAllBut(OutputType::Html);
      ...
      if (cd && !md->name().isEmpty() && !md->isAnonymous())
      {
        ...
        // create a link to the documentation
        {
          ...
          memberWritten = TRUE;
                 ...
                 (protectionLevelVisible(md->protection()) || md->isFriend())) // no documentation,
            ...
            memberWritten)
        {
          ...
          if (lang == SrcLangExt_VHDL)
          {
            ...
            sl.push_back("related");
          else
          {
            if (Config_getBool(INLINE_INFO) && md->isInline())
              sl.push_back("inline");
            if (md->isExplicit())
              sl.push_back("explicit");
            if (md->isMutable())
              sl.push_back("mutable");
            if (prot == Protection::Protected)
              sl.push_back("protected");
            else if (prot == Protection::Private)
              sl.push_back("private");
            else if (prot == Protection::Package)
              sl.push_back("package");
            if (virt == Specifier::Virtual && getLanguage() != SrcLangExt_ObjC)
              sl.push_back("virtual");
            else if (virt == Specifier::Pure)
              sl.push_back("pure virtual");
            if (md->isStatic())
              sl.push_back("static");
            if (md->isSignal())
              sl.push_back("signal");
            ...
            if (md->isOptional())
              sl.push_back("optional");
            if (md->isAttribute())
              sl.push_back("attribute");
            if (md->isUNOProperty())
              sl.push_back("property");
            if (md->isReadonly())
              sl.push_back("readonly");
            if (md->isBound())
              sl.push_back("bound");
            if (md->isRemovable())
              sl.push_back("removable");
            if (md->isConstrained())
              sl.push_back("constrained");
            if (md->isTransient())
              sl.push_back("transient");
            if (md->isMaybeVoid())
              sl.push_back("maybevoid");
            if (md->isMaybeDefault())
              sl.push_back("maybedefault");
            if (md->isMaybeAmbiguous())
              sl.push_back("maybeambiguous");
          }
          bool firstSpan = true;
          for (const auto &s : sl)
          {
...
bool ClassDefImpl::isLinkableInProject() const
{
  ...
  else
  {
    ...
    //       !isReference());
    return !isArtificial() && !isHidden() &&       /* not hidden */
...
void ClassDefImpl::mergeMembers()
{
  ...
  for (const auto &bcd : baseClasses())
  {
    ...
    if (bClass)
    {
      ...
      bClass->mergeMembers();
      if (bClass->getLanguage() == SrcLangExt_Python)
        continue; // python does not have member overloading, see issue 8480
        ...
        // visited via another branch in the class hierarchy.
        {
          for (auto &srcMi : *srcMni)
          {
            ...
            if (!found && srcMd->protection() != Protection::Private && !srcMd->isFriend())
            {
              ...
              if (virt == Specifier::Normal && bcd.virt != Specifier::Normal)
                virt = bcd.virt;
                                               ...
                                               int startLine, int startColumn, const QCString &templSpec, bool &freshInstance) const
{
  ...
  auto it = std::find_if(m_impl->templateInstances.begin(),
                         ...
                         [&templSpec](const auto &ti)
                         { return templSpec == ti.templSpec; });
...
void ClassDefImpl::sortMemberLists()
{
  ...
  std::sort(m_impl->innerClasses.begin(),
            ...
            {
              return Config_getBool(SORT_BY_SCOPE_NAME) ? qstricmp(c1->name(), c2->name()) < 0 : qstricmp(c1->className(), c2->className()) < 0;
            });
}
  ...
  if (getLanguage() != SrcLangExt_VHDL) // use specific declarations function
  {
    ...
    if (inheritedFrom)
    {
      for (const auto &mg : m_impl->memberGroups)
      {
        ...
        if (lt2 != -1)
          count += mg->countGroupedInheritedMembers(static_cast<MemberListType>(lt2));
      }
    }
    bool inlineInheritedMembers = Config_getBool(INLINE_INHERITED_MEMB);
...
CodeSymbolType ClassDefImpl::codeSymbolType() const
{
  ...
  case Struct:
    ...
    break;
  case Union:
    return CodeSymbolType::Union;
    break;
  case Interface:
    return CodeSymbolType::Interface;
    break;
  case Protocol:
    return CodeSymbolType::Protocol;
    break;
  case Category:
    return CodeSymbolType::Category;
    break;
  case Exception:
    return CodeSymbolType::Exception;
    break;
  case Service:
    return CodeSymbolType::Service;
    break;
  case Singleton:
    return CodeSymbolType::Singleton;
    break;
  default:
    break;
  }
...
Protection classInheritedProtectionLevel(const ClassDef *cd, const ClassDef *bcd, Protection prot, int level)
{
  ...
  else if (prot != Protection::Private)
  {
    for (const auto &bcdi : cd->baseClasses())
    {
      ...
      if (baseProt == Protection::Private)
        prot = Protection::Private;
      else if (baseProt == Protection::Protected)
        prot = Protection::Protected;
    }
  }
exit:
