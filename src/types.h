...
enum SrcLangExt
{
  ...
  SrcLangExt_Verilog  = 0x08000, // no longer supported
...
constexpr const char *codeSymbolType2Str(CodeSymbolType type)
{
  switch (type)
  {
    ...
    case CodeSymbolType::Module   :  return "module";
...
/* 60 */ TSPEC(MaybeDefault)      TSPEC(MaybeAmbiguous)  TSPEC(Published)         TSPEC(Module)       TSPEC(Package)            \
/* 65 */ TSPEC(Program)           TSPEC(Checker)         TSPEC(Configuration)     TSPEC(Class)        
