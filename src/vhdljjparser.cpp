...
static void printEntries(std::shared_ptr<Entry> ce);
...
void VHDLOutlineParser::handleCommentBlock(const QCString &doc1, bool brief)
{
 ...
 std::cout<<"\n"<<processedDoc<<" \n----\n"<<doc<<"\n";
  ...
  Entry *pm=s->current.get();
  std::cout<<pm->bodyLine<<" "<<pm->name<<"\n";
...
void VHDLOutlineParser::addCompInst(const char *n, const char* instName, const char* comp,int iLine)
{
  ...
  if (s->lastCompound)
  {
    ...
    if (true) // !findInstant(current->type))
    {
      ...
      // TODO: end protect with mutex std::cout<<" doc:"<<rt->doc.data()<<" at line: "<<rt->docLine<<"\n";
    }
...
void VHDLOutlineParser::setEndBodyLine(int line,int endline)
{
  const std::shared_ptr<Entry> ce = getEntryAtLine(line);
  if(ce!=NULL){
    ce.get()->endBodyLine=endline;
   }
}

// returns the vhdl parsed types at line xxx
const std::shared_ptr<Entry>  VHDLOutlineParser::getEntryAtLine(int line)
{
  VhdlParser::SharedState *s = &p->shared;
  std::shared_ptr<Entry> ce=0;
  if(s->lastEntity !=0)
   ce=s->lastEntity;
  ...
  if(ce==NULL)
   ...
   if(ce->startLine==line)
     return ce;

  for (const auto &rt : ce->children())
  {
    if (rt->startLine==line || rt->bodyLine==line)
    {
      return rt;
    }
  }
...
void printEntries(std::shared_ptr<Entry> ce)
{
  for (const auto &rt : ce->children())
  {
    std::cout<<" name: "<<rt->name.data()<<" "<<rt->startLine<<" "<<rt->bodyLine<<"\n";
    if (!rt->brief.isEmpty())
    {
      std::cout<<" brief:"<<rt->brief.data()<<" at line: "<<rt->briefLine<<"\n";
    }

   if (!rt->doc.isEmpty())
    {
      std::cout<<" doc:"<<rt->doc.data()<<" at line: "<<rt->docLine<<"\n";
    }
